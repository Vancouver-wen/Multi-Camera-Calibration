...
class MyCalibrate():
    ...
    def standardize_json(self,rmat,tvec):
        ...
        for index in range(self.config.cam_num):
            ...
            # cam_real_param["R"]=np.matmul(origin_R,transform_R).tolist()
            # cam_real_param["T"]=(np.matmul(np.linalg.inv(transform_R),-transform_T)+origin_T).tolist()
            # (x,y,z) -> (x,z,y)
            cam_real_param["T"][0],cam_real_param["T"][1],cam_real_param["T"][2]=(
                cam_real_param["T"][0],
                cam_real_param["T"][1],
                cam_real_param["T"][2],
            )
...
if __name__=="__main__":
    ...
    myplot3d.draw_cameras_pose(real_calibrations)
    myplot3d.show()
