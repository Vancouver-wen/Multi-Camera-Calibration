...
class MyCalibration():
    ...
    使用 start_calibrate
    """
    def __init__(self,config,yaml_path,imgfolder_path):
        self.config=config
    ...
    def prepare_for_calibration(self,):
        ...
        self.imgfolder_path=os.path.abspath(self.imgfolder_path)
    ...
    def start_calibrate(self,myCapture):
        """
        该方法返回一个 相机校准json 的绝对地址
        """
        self.start_intrinsic_calibrate() # 注释 以方便 debug
        rmat,tvec=self.start_extrinsic_calibrate(myCapture)
    ...
    def start_extrinsic_calibrate(self,myCapture):
        ...
        # 计算 cam1的 真实 相机外参
        rmat,tvec=self.cam1_extrinsic_calibrate(cams_real_param[0],myCapture)
    ...
    def cam1_extrinsic_calibrate(self,cam1_param,myCapture):
        ...
        if self.config.recollect==True:
            print("从相机中读取第一个相机的图像用于绝对外参的校准 ..")
            # 因为需要使用鼠标确定几个点位，所以不允许调整图像大小
            cv2.namedWindow("cam0_frame", cv2.WINDOW_AUTOSIZE)
            print("按s保存。因为需要使用鼠标确定几个点位，所以不允许调整图像窗口大小 ..")
            while True:
                frames=myCapture.get_frames()
                frame=frames[0]
                cv2.imshow("cam0_frame",frame)
                key=cv2.waitKey(1)
                if key==ord('s'):
                    img_before=frame
                    break
